...
export async function apiKeyAuth(c: Context, next: Next) {
  ...
  if (!header?.startsWith('Bearer mp-')) {
    return c.json({ error: { message: 'missing or invalid api key', type: 'auth_error' } }, 401);
  }
  const raw = header.slice('Bearer '.length).trim();
  const hash = createHash('sha256').update(raw).digest('hex');

  const [key] = await db
    .select({ id: s.apiKeys.id, userId: s.apiKeys.userId, status: s.apiKeys.status })
    .from(s.apiKeys)
    .where(and(eq(s.apiKeys.keyHash, hash), eq(s.apiKeys.status, 'active')))
    .limit(1);

  if (!key) {
    return c.json({ error: { message: 'invalid or revoked api key', type: 'auth_error' } }, 401);
  }
  ...
  if (!user || user.suspended) {
    return c.json({ error: { message: 'account suspended', type: 'auth_error' } }, 403);
  }
